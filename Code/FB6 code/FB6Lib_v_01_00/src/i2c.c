...
volatile uint8_t I2CMasterBuffer[I2C_PORT_NUM][I2C_BUFSIZE];
volatile uint8_t I2CSlaveBuffer[I2C_PORT_NUM][I2C_BUFSIZE];
...
void ResetI2C0(void)
{
	uint32_t i=0;
	/*
	 * I2C0 SDA0-> P0.27  Input
	 *	    SCL0-> P0.28  Output
	 */
	LPC_GPIO0->FIOMASK&=0xE7FFFFFF;

	LPC_GPIO0->FIODIR&=0xE7FFFFFF;
	LPC_GPIO0->FIODIR|=0x10000000;

	while(((LPC_GPIO0->FIOPIN & 0x08000000)!=0x08000000))
	{
		//LPC_GPIO0->FIOSET = 0x08000000;
		for(i=0;i<10000;i++);
		LPC_GPIO0->FIOSET = 0x10000000;		//SCL High
		for(i=0;i<1000;i++);
		LPC_GPIO0->FIOCLR = 0x10000000;		//SCL Low
		for(i=0;i<1000;i++);
		LPC_GPIO0->FIOSET = 0x10000000;		//SCL High
		for(i=0;i<10000;i++);
	}
...
void ResetI2C1(void)
{
	...
	/*
	 * I2C1 SDA1-> P0.19 Input
	 *	    SCL1-> P0.20 OutPut
	 */
	LPC_GPIO0->FIOMASK&=0xFFE7FFFF;
	LPC_GPIO0->FIODIR&=0xFFE7FFFF;
	LPC_GPIO0->FIODIR|=0x00100000;

	while(((LPC_GPIO0->FIOPIN & 0x00080000)!=0x00080000))
	{
		//LPC_GPIO0->FIOSET = 0x00080000;
		for(i=0;i<10000;i++);
		LPC_GPIO0->FIOSET = 0x00100000;		//SCL High
		for(i=0;i<1000;i++);
		LPC_GPIO0->FIOCLR = 0x00100000;		//SCL Low
		for(i=0;i<1000;i++);
		LPC_GPIO0->FIOSET = 0x00100000;		//SCL High
		for(i=0;i<10000;i++);
	}
